...
public class MultiWebServer {
    public static String FileType(String fileType){
        ...
        return contentType.get(fileType);
    }
        ...
        while (true) {     //http1.1 ：html中的图片也会发送一次http请求，所以会产生两次请求，当网页中的图片更多时，会产生更多http请求头
            ...
            try {
                BufferedReader bufferedReader0 = new BufferedReader(
                        new InputStreamReader(new FileInputStream("D:\\computer networking lab1\\confi.txt"),"utf-8"));
                String confiIP = bufferedReader0.readLine();
                int confiPort = Integer.parseInt(bufferedReader0.readLine());
                String confiPath = bufferedReader0.readLine();
                String base_url = confiPath;
                ServerSocket severSocket = new ServerSocket(confiPort,50, Inet4Address.getByName(confiIP));    //创建socket
                System.out.println("--------------------------------------");
                System.out.println("等待请求");
                Socket socket = severSocket.accept();   //接收到客户端发来的socket，准备解析
                System.out.println("监听中...\r\n");

                InputStream inputStream = socket.getInputStream();  //注意：要从socket中读取信息，而不是seversocket
                BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream));
                String line = reader.readLine();
                System.out.println("请求行：" + line);   //打印首部行的第一行
                String url = line.substring(5, line.indexOf("HTTP") - 1);   //得到第一行想要获取的文件名称
                String fileType = url.substring(url.indexOf("."));  //获取文件类型
                OutputStream outputStream;

                InetAddress localhost = InetAddress.getLocalHost();
                String ip = localhost.getHostAddress();
                System.out.println("客户端ip："+ip + "\r\n" +"客户端端口号："+ confiPort+"\r\n");
                if (new File(base_url + url).exists()) { //获取的文件存在
                    inputStream = new FileInputStream(base_url + url);
                    outputStream = socket.getOutputStream();
                    byte[] buffer = new byte[400 * 1024];
                    BufferedReader reader1 = new BufferedReader(new FileReader(base_url + url));
                    ...
                    result.append("Content-Type:" + FileType(fileType) + " \r\n");
                    System.out.println(result.toString()+"Content-Length:"+new File(base_url + url).length());
                    result.append("\r\n");
                    ...
                    System.out.println("error:想要获取的文件不存在\r\n");
                    outputStream = socket.getOutputStream();
                    StringBuffer error = new StringBuffer();
                    error.append("HTTP/1.1 400 file not found \r\n");
                    error.append("Content-Type:text/html \r\n");
                    error.append("Content-Length:25 \r\n").append("\r\n");
                    error.append("<h1 >404 File Not Found..</h1>");
                    outputStream.write(error.toString().getBytes());
                    System.out.println(error.toString());
                    outputStream.flush();
                }

                System.out.println("客户端请求已经处理完毕");

                severSocket.close();    //将打开的开关都关掉
